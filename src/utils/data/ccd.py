...
from src.utils.data.substructure_perms import get_substructure_perms
...
def biotite_load_ccd_cif() -> pdbx.CIFFile:
    ...
    """
    return pdbx.CIFFile.read(f"{os.getenv('DATA_ROOT_DIR')}/components.v20240608.cif")
...
def get_component_rdkit_mol(ccd_code: str) -> Union[Chem.Mol, None]:
    ...
    rdkit_mol_pkl = Path(f"{os.getenv('DATA_ROOT_DIR')}/components.v20240608.cif.rdkit_mol.pkl")
