...
from src.utils.data import ccd
...
class MMCIFParser:
    ...
    def get_bioassembly(
        ...
        # Add reference information for atoms
        atom_array = AddAtomArrayAnnot.add_ref_info(atom_array)
...
class AddAtomArrayAnnot(object):
    ...
    def add_token_mol_type(
        ...
        for start, stop in zip(starts[:-1], starts[1:]):
            entity_id = atom_array.label_entity_id[start]
            if entity_id not in sequences.keys():
                # non-poly is ligand
    ...
    def add_centre_atom_mask(atom_array: AtomArray) -> AtomArray:
        ...
        if hasattr(atom_array, "atomized"):
            std_res = np.isin(atom_array.res_name, res_name) & (atom_array.mol_type != "ligand") & (atom_array.atomized == False)
        else:
    ...
    @staticmethod
    def add_distogram_rep_atom_mask(atom_array: AtomArray, motif_scaffolding: bool) -> AtomArray:
        """
        ...
        if motif_scaffolding:
            prot_gly_ca = prot_gly_ca | ((atom_array.res_name == 'UNK') & (atom_array.atom_name == "CA"))
        ...
        # for nucleotide design res
        design_nuc = np.isin(atom_array.mol_type, ["dna", "rna"]) & (
            atom_array.atom_name == r"C1'"
        ) & (~atom_array.condition_token_mask)
    ...
    def add_plddt_m_rep_atom_mask(atom_array: AtomArray) -> AtomArray:
        ...
        """
        if hasattr(atom_array, "atomized"):
            std_res = np.isin(atom_array.res_name, list(STD_RESIDUES.keys())) & (atom_array.mol_type != "ligand") & (atom_array.atomized == False)
        else:
    ...
    def add_tokatom_idx(atom_array: AtomArray) -> AtomArray:
        ...
        for atom in atom_array:
            ...
            if atom.mol_type == "ligand" or atom_name_position is None:
                ...
                # design nucleotide with condition_token_mask=0, N1 as representative atom
                if (~atom.condition_token_mask) and np.isin(atom.mol_type, ["dna", "rna"]) & (atom.atom_name == 'N'):
                    tokatom_idx = atom_name_position['N1']
                else:
                    try:
                        tokatom_idx = atom_name_position[atom.atom_name]
                    except:
                        raise ValueError(
                            f"Cannot find atom_name {atom.atom_name} in res_name {atom.res_name} for tokatom_idx. Please check standard atom name tabel"
                        )
    ...
    def add_res_perm(
        ...
        """
        condition_token_mask = atom_array.condition_token_mask
        ...
        for start, stop in zip(starts[:-1], starts[1:]):
            ...
            if not res_dict or not condition_token_mask[start:stop].all():
        ...
        assert len(atom_array) == len(str_res_perm)
        atom_array.set_annotation("res_perm", str_res_perm)

        return atom_array

    @staticmethod
    def add_ref_info(atom_array: AtomArray) -> AtomArray:
        """
        ...
        assert (
            ...
            == len(ref_charge)
            == len(ref_mask)
        ), f"{len(atom_array)=}, {len(ref_pos)=}, {len(ref_charge)=}, {len(ref_mask)=}"
    ...
    @staticmethod
    def add_atomize_residue(atom_array: AtomArray, atomize_config: dict = {}) -> AtomArray:
        """
        Add an 'atomized' annotation to the atom array for specified residues.

        This function identifies atoms based on their entity ID and residue ID,
        then sets a boolean flag 'atomized' to True for them.

        Args:
            atom_array (AtomArray): The atom array to be modified.
            atomize_config (dict): A dictionary specifying which residues to atomize.
                                The format is {entity_id: [res_id_1, res_id_2, ...]}.

        Returns:
            AtomArray: The atom array with the 'atomized' annotation updated.
        """

        atom_array.set_annotation("atomized", np.zeros(len(atom_array), dtype=bool))
        atomized_annotation = atom_array.get_annotation("atomized")

        for label, res_idxs in atomize_config.items():
            selection_mask = (atom_array.label_entity_id == label) & \
                            (np.isin(atom_array.res_id, res_idxs))
            atomized_annotation[selection_mask] = True
        
        atom_array.set_annotation("atomized", atomized_annotation)
        return atom_array
