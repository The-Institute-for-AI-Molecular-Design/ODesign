...
from src.utils.data.constants import DNA_STD_RESIDUES, PRO_STD_RESIDUES, RNA_STD_RESIDUES
...
class CIFWriter:
    ...
    def save_to_cif(
        ...
        # Use condition_token_mask as occupancy values instead of default ones
        atom_site["occupancy"] = np.asarray(
            self.atom_array.condition_token_mask, dtype=np.int32
        )
...
def make_dummy_feature(
    ...
    if "msa" in dummy_feats:
        ...
        features_dict["msa"] = torch.nonzero(restype)[:, 1].unsqueeze(
            0
        )
...
def exponential_decay(x, peak_pos=4, decay_rate=0.3): 
    weights = torch.zeros_like(x, dtype=torch.float32)
    mask1 = x <= peak_pos 
    weights[mask1] = x[mask1] / peak_pos 
    mask2 = x > peak_pos 
    weights[mask2] = torch.exp(-decay_rate * (x[mask2] - peak_pos)) 
