...
import torch
from src.api.model_interface import GroundTruth
from src.utils.model.rmsd import rmsd, self_aligned_rmsd
from src.utils.permutation.chain_permutation.utils import (
    apply_transform,
    get_optimal_transform,
    num_unique_matches,
)
from src.utils.permutation.utils import Checker
...
def _correct_symmetric_chains_for_one_sample(
    ...
    **kwargs,
    ...
    if not permute_label:
        ...
        """
        assert pred_dict["coordinate"].size(-2) == ground_truth["coordinate"].size(
            -2
        )
    ...
    if permute_label:
        ...
        output_dict = {
            "coordinate": ground_truth.coordinate[indices, :],
            "coordinate_mask": ground_truth.coordinate_mask[indices],
        }
        # Permute extra label features, along the last dimension.
        output_dict.update(
            {
                k: ground_truth[k][..., indices]
...
class MultiChainPermutation(object):
    ...
    def process_input(
        ...
        Args:
            pred_dict (dict[str, torch.Tensor]): A dictionary containing
                masked_asym_ids, entity_mol_id, mol_id, mol_atom_index, pae_rep_atom_mask, coordinate, is_ligand.
                All Tensors have shape = [N_cropped_atom]
            ground_truth (GroundTruth): A dictionary containing
        ...
        # Get masked chain IDs to exclude from permutation
        masked_asym_ids = pred_dict.get('masked_asym_ids', [])
        num_masked_asym_ids = len(masked_asym_ids) 
        self.masked_asym_ids = masked_asym_ids

        log_dict = {}

        for key in ["entity_mol_id", "mol_id", "mol_atom_index"]:
            pred_dict[key] = pred_dict[key].long()
            ground_truth[key] = ground_truth[key].long()

        # get original unpermuted match
        pred_mol_id = set(torch.unique(pred_dict["mol_id"]).tolist())
        label_mol_id = set(torch.unique(ground_truth.mol_id).tolist())
        if pred_mol_id.intersection(label_mol_id) != pred_mol_id:
            ...
            assert pred_dict["coordinate"].size(-2) == ground_truth.coordinate.size(-2)
            self.unpermuted_match = self.check_pattern_and_create_mapping(
                pred_dict["mol_id"], ground_truth.mol_id
            )
        else:
        ...
        if len(torch.unique(ground_truth.entity_mol_id)) == len(
            torch.unique(ground_truth.mol_id) 
        ) or (
            torch.unique(ground_truth.mol_id).size(0) <= num_masked_asym_ids
            ):
        ...
        n_label_chain = len(torch.unique(ground_truth.mol_id))
        if n_label_chain > 20:
        ...
        self.label_token_dict, self.label_asym_dict = self._parse_atom_feature_dict(
            self.dict_of_interested_keys(ground_truth),
            rep_atom_mask=ground_truth.pae_rep_atom_mask,
        )
    ...
    def find_anchor_asym_chain_in_predictions(self) -> tuple[int]:
        ...
        # Filter out masked chains and chains with fewer than 4 tokens
        valid_asyms = [
            asym_id for asym_id, l in asym_to_asym_length.items() 
            if l >= 4 and asym_id not in self.masked_asym_ids
        ]
        ...
        # Handle edge case: masked chains may result in empty entity_to_asym_count
        if len(entity_to_asym_count) == 0:
            return None
    ...
    def compute_best_match_heuristic(self):
        ...
        # Handle edge case: masked chains may result in no valid anchor
        if anchor_pred_asym_id is None:
            return self.unpermuted_match

        anchor_entity_id = self.pred_token_dict["asym_to_entity"][anchor_pred_asym_id]  # entity level
        ...
        for anchor_k in candidate_anchors:  # chain(asym) level
            ...
            # Skip masked chains: ensure they remain unpermuted
            if pred_anchor in self.masked_asym_ids or gt_anchor in self.masked_asym_ids:
                gt_anchor = pred_anchor
            ...
            # Exclude masked chains from candidate ground truth chains
            candidate_gt_asym_id = [
                k for k in self.label_asym_dict 
                if k != gt_anchor and k not in self.masked_asym_ids
            ]
            ...
            while len(to_be_matched) > 0:
                ...
                # Masked chains remain unpermuted (map to themselves)
                if cur_pred_asym_id in self.masked_asym_ids:
                    matched_asym[cur_pred_asym_id] = cur_pred_asym_id
                    continue
    ...
    def build_permuted_indice(
        pred_dict: dict, ground_truth: GroundTruth, best_match: dict[int, int]
    ):
        """
        ...
        N_label_atom = ground_truth["mol_id"].size(0)
    ...
    def aligned_rmsd(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            aligned_rmsd, _, _, _ = self_aligned_rmsd(
                ...
                true_pose=ground_truth["coordinate"][indices, :].to(torch.float32),
                atom_mask=ground_truth["coordinate_mask"][indices],
    ...
    def __call__(
        ...
        match, has_sym_chain = self.process_input(
            pred_dict, ground_truth, max_num_chains
        )
        ...
        if match is not None:
            ...
            """
            indices = self.build_permuted_indice(pred_dict, ground_truth, match)
        ...
        permuted_indices = self.build_permuted_indice(
            pred_dict, ground_truth, best_match
        )
        ...
        unpermuted_indices = self.build_permuted_indice(
            pred_dict, ground_truth, self.unpermuted_match
        )

        permuted_rmsd = self.aligned_rmsd(pred_dict, ground_truth, permuted_indices)
        unpermuted_rmsd = self.aligned_rmsd(
            pred_dict, ground_truth, unpermuted_indices
        )
        ...
        if pred_dict["coordinate"].size(-2) == ground_truth["coordinate"].size(-2):
